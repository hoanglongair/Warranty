    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      {!isArcTestnet && realConnection && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mb-6 flex items-center gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4"
        >
          <AlertCircle className="h-5 w-5 flex-shrink-0 text-amber-400" />
          <div className="flex-1">
            <p className="text-sm font-medium text-amber-100">
              Bạn đang ở chain khác Arc Testnet (chain ID: {chainId})
            </p>
            <p className="text-xs text-amber-200/80">
              Hãy chuyển sang Arc Testnet để sử dụng đầy đủ tính năng.
            </p>
          </div>
          <button
            onClick={handleSwitchChain}
            className="rounded-lg bg-amber-500/30 px-3 py-1.5 text-xs font-semibold text-amber-100 hover:bg-amber-500/50"
          >
            Chuyển chain
          </button>
        </motion.div>
