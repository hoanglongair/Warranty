...
import {
  ...
  ExternalLink, Filter, Clock, RefreshCcw, AlertCircle, LogOut,
  Globe, ChevronDown
} from "lucide-react";
import { useWalletStore } from "@/store/wallet-store";
import { formatCurrency, formatAddress } from "@/lib/utils";
import { WalletModal } from "@/components/wallet/wallet-modal";
import { NetworkModal } from "@/components/wallet/network-modal";
...
export default function WalletPage() {
  ...
  const { connected, transactions, balance, address, provider, chainId, realConnection, setBalance, setChainId, setError, disconnect } =
    useWalletStore();

  const isArcTestnet = chainId === ARC_CHAIN_ID;
  const currentNetwork = getChainConfig(chainId || 0);
  const symbol = currentNetwork ? currentNetwork.symbol : isArcTestnet ? "USDC" : "ETH";
  const ethUsdPrice = 2950;
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Không thể làm mới.";
      setError(message);
    } finally {
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      >
        <div>
          ...
          <p className="mt-3 max-w-2xl text-lg text-white/60">
            Manage your crypto assets and transaction history across EVM chains.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {realConnection && (
            <button
              ...
              className="btn-secondary px-3 py-2 text-xs flex items-center gap-2 border-violet-500/30 bg-violet-500/10 text-violet-200 hover:bg-violet-500/20 transition-all"
              title="Đổi mạng / chain"
            >
          ...
          <button
            ...
            className="btn-secondary px-3 py-2 text-xs text-red-300 hover:text-red-200"
      ...
      {!isArcTestnet && realConnection && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4"
        >
          <div className="flex items-center gap-3">
            <AlertCircle className="h-5 w-5 flex-shrink-0 text-amber-400" />
            <div>
              <p className="text-sm font-medium text-amber-100">
                Bạn đang ở {currentNetwork ? currentNetwork.name : `chain ID: ${chainId}`}
              </p>
              <p className="text-xs text-amber-200/80">
                Hãy chuyển sang Arc Testnet hoặc chọn mạng thích hợp để giao dịch.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className="rounded-lg bg-amber-500/30 px-3 py-1.5 text-xs font-semibold text-amber-100 hover:bg-amber-500/50 transition-colors"
            >
              Chuyển về Arc Testnet
            </button>
            <button
              onClick={() => setNetworkModalOpen(true)}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-white/40 capitalize">
                  {provider} • {realConnection ? "Live" : "Demo"} • Chain {chainId}
                </p>
              </div>
            </div>
        ...
        >
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <select className="w-full h-12 rounded-xl border border-white/10 bg-white/[0.04] px-4 text-white outline-none focus:border-violet-500/50">
                  <option value={symbol.toLowerCase()}>{symbol}</option>
            ...
            <button className="btn-primary w-full" disabled>
              ...
              Gửi (cần tích hợp giao dịch thực)
            </button>
            <p className="text-center text-xs text-white/40">
              Tính năng gửi giao dịch thật đang được phát triển. Bạn vẫn có thể xem balance và chain từ ví của mình.
            </p>
          </div>
        </motion.div>
      )}
